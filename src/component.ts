...
export class AuditLogComponent implements Component {
  constructor(
    ...
    @inject(TenantUtilitiesBindings.DisableTenantGuard, {optional: true})
    private readonly disableTenantGuard?: IDisableTenantGuard,
  ) {
    this.application.component(TenantUtilitiesComponent);
    if (this.disableTenantGuard?.disable) {
